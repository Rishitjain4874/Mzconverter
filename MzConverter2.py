...
class AsciiDeclaration(ASTNode):
    ...
    def __repr__(self):
        if not self.additional_attributes or self.additional_attributes == [',']:
            return f"AsciiDeclaration(name={self.name}, terminator={self.terminator}, attri_type={self.attri_type})"
        else:
            return f"AsciiDeclaration(name={self.name}, terminator={self.terminator}, attri_type={self.attri_type}, additional_attributes={self.additional_attributes})"
...
class Identifier(ASTNode):
    ...
    def __repr__(self):
        return f"Identified_by(tag={self.tag}, attributes={self.attributes})"
...
class Parser:
    ...
    def parse_identified_by(self):
        ...
        if self.pos < len(self.tokens) and self.tokens[self.pos] == 'ascii':
            ...
            if self.pos < len(self.tokens) and self.tokens[self.pos] == 'ascii':
                ascii_declaration1.append(self.parse_ascii_declaration())
        return Identifier(identifier, ascii_declaration1)
    ...
    def parse_ascii_declaration(self):
        ...
        ascii_decl = AsciiDeclaration(name, terminator, attri_type)
                ...
                for attr in declaration.attributes:
                    if isinstance(attr, AsciiDeclaration):
                        ...
                        if terminator_value:
                            ...
                            terminator_value_str = "''"
                        if attri_value == 'str':
                            go_code += f"\t{declaration.tag} string `mzConverter: terminated_by({terminator_value_str})\"`\n"
                        elif 'int' in attri_value:
...
def parse_ast(ast_str):
    ...
    identifed_by_pattern = r"Identified_by\(tag=(\w+), attributes=\[(AsciiDeclaration\(name=([^,]+),\s*terminator=Terminator\(value=(0x[0-9A-Fa-f]+)\),\s*attri_type=(str|\('int', 'base\d+'\)|\('long', 'base\d+'\))\))\]\)"
    ...
    for match in ast_matches:
        ...
        for declaration_match in re.finditer(identifed_by_pattern, declarations_str):
            ...
            ascii_declaration_name = declaration_match.group(3)
            terminator_value = declaration_match.group(4).strip()
            attri_type = declaration_match.group(5)
            ascii_declaration = AsciiDeclaration(name=ascii_declaration_name, terminator=Terminator(value=terminator_value), attri_type=attri_type)
            identifier = Identifier(tag=tag, attributes=[ascii_declaration])
